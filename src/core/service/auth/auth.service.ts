...
export class AuthService {
    ...
    return this.http.post<any>(`${this.baseUrl}/login`, request).pipe(
      tap(response => {
        if (response.token) {
          ...
          this.storage.setItem('instituicaoId', decoded.instituicaoId.toString());
          this.storage.setItem('instituicaoNome', decoded.instituicaoNome);
  ...
  logout(): void {
    ...
    this.storage.removeItem('instituicaoNome');
  ...
  public getInstituicaoId(): number | null {
    const decoded = this.decodeToken();
    return decoded?.instituicaoId || null;
  }
  
  public getInstituicaoNome(): string | null {
    const decoded = this.decodeToken();
    return decoded?.instituicaoNome || null;
  }
